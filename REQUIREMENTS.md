...
- 対象:
  ...
  - スマホWebアプリ（セッション受け取り、サブスク状態判定、運動検知、解除送信）
...
### FR-07 スマホ認証
- 認証は拡張側で実施し、スマホ側はセッション実行に特化する。
- スマホ側の会員判定は `device_links`（`device_id`）を参照して行う。
- スマホ側ログイン導線は通常フローでは利用しない（拡張→QR起動を正規導線とする）。
...
- ロック時間は `20` 分固定
  - 運動回数は `5〜30` 回で調整可能
  - ブロックサイトはプリセットのみ最大 `5` 件まで（カスタムドメインはProのみ）
  - スケジュールは `WEEKDAYS / EVERYDAY` の2択（時間帯指定はProのみ）
  - `ADULT BLOCK` は利用可能
- Pro権限:
  - 上記制限を解除し、詳細設定を利用可能
...
- 決済戻り先は `billing-return.html` で受け、拡張側プラン状態の再同期を行う。

### FR-10 セッション開始
- 手入力またはQRスキャンでセッションIDを取得し開始できる。
- URLパラメータ `session`, `target` を受け取り可能とする。
- セッションID が `SET-` または `CFG-` で始まる場合、目標回数を15回に固定する。
...
- MediaPipe Pose を用いて以下3種目を検知する:
  ...
  - SIT-UP
- Freeは `SQUAT` 固定とする。
- Proはスマホアプリ側で種目（SQUAT/PUSH-UP/SIT-UP）を選択できる。
- 目標回数到達時に完了画面へ遷移する。
...
- `Manage Subscription` は拡張フローを維持するため、新規ウィンドウで開く。
...
## 6. 外部インターフェース要件
- Supabase Auth
  - `signInWithOAuth` (Google, 拡張ログイン導線)
  - `signOut`
- Supabase REST
...
- AC-08: `free` 制限値（ロック時間/運動回数/サイト上限/簡易スケジュール）が仕様どおり適用される。
...
補足:
- FR-07 は拡張の Google OAuth を正規導線とし、スマホはセッション起動を前提とする。
