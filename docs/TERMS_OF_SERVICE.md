# THE TOLL Terms of Service (Draft)

Last updated: 2026-02-13

This Terms of Service ("Terms") governs your use of THE TOLL Chrome extension and smartphone web app (the "Service").
By using the Service, you agree to these Terms.

## 1. Eligibility
- You must be legally able to enter into a binding agreement in your jurisdiction.
- If you use the Service on behalf of an organization, you confirm you have authority to bind that organization.

## 2. Service Overview
- THE TOLL helps users restrict access to selected websites until exercise tasks are completed via smartphone.
- Paid subscription features are provided through Stripe billing.

## 3. Account and Access
- You may sign in using Google authentication.
- You are responsible for activity under your account and for keeping your device access secure.

## 4. Subscription and Billing
- Paid plans are billed through Stripe.
- Pricing, billing interval, and currency are shown during checkout.
- Plan changes are reflected at the next billing timing.
- If you choose cancellation at period end, access remains active until the current billing period ends.
- If you cancel immediately, paid access may end immediately.
- Refund policy: In principle, fees are non-refundable after payment. Exceptions apply only where refunds are required by applicable law or explicitly approved by us.

## 5. Acceptable Use
You agree not to:
- Reverse engineer, abuse, disrupt, or attempt unauthorized access to the Service.
- Use the Service for unlawful activity.
- Interfere with security, availability, or integrity of Service systems.

## 6. Third-Party Services
- The Service relies on third-party services including Google, Stripe, Supabase, and browser platforms.
- We are not responsible for third-party outages or policy changes.

## 7. Service Changes
- We may modify features, availability, or technical requirements.
- We may update these Terms and will publish the updated date.

## 8. Disclaimer
- The Service is provided "as is" and "as available".
- We do not guarantee uninterrupted or error-free operation.

## 9. Limitation of Liability
- To the maximum extent allowed by law, we are not liable for indirect, incidental, special, or consequential damages.

## 10. Termination
- You may stop using the Service at any time.
- We may suspend or terminate access for violations of these Terms or abuse of the Service.

## 11. Governing Law
- These Terms are governed by the laws of Japan.
- Any dispute arising out of or in connection with the Service shall be subject to the exclusive jurisdiction of the Tokyo District Court as the court of first instance.

## 12. Contact
- Operator name: THE TOLL Operations Office
- Contact email: support@example.com
